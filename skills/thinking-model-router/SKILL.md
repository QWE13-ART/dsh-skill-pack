...
version: 1.0.0
description: 模型路由（选思维框架、按问题类型匹配）。When unsure which thinking skill fits, map domain and problem type, then return NONE or one primary skill by default (at most three complementary).
---
...
The router outputs bare slugs (e.g. `id: five-whys-plus`). Invoke a routed skill with the Skill tool using the exact ID `thinking-<slug>` (e.g. `thinking-five-whys-plus`) — never invent a slug not cited in Procedure. `NONE` means no invocation.
