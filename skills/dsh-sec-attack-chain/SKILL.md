...
name: dsh-sec-attack-chain
version: 1.0.0
description: 攻击链路编排（多阶段攻击路径规划、渗透测试编排）。Use for authorized multi-stage attack-path planning and orchestration when a task spans reconnaissance, initial access, privilege escalation, lateral movement, or impact assessment. Route single-stage tasks directly to their specialist skill.

---
> ⚠️ 本技能为攻击/渗透方法论文档，仅限**已授权**场景使用。实际执行渗透测试前必须先取得用户明确同意。
